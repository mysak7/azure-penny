...
def _get_resource_mgmt_client() -> ResourceManagementClient:
    ...
    if _resource_mgmt_client is None:
        ...
        _resource_mgmt_client = ResourceManagementClient(
...
def _get_compute_mgmt_client() -> ComputeManagementClient:
    ...
    if _compute_mgmt_client is None:
        ...
        _compute_mgmt_client = ComputeManagementClient(
            credential, AZURE_SUBSCRIPTION_ID
        )
...
_FILES_PREMIUM_PRICES: dict[str, float] = {
    ...
    "ukwest": 0.22,
    "swedencentral": 0.21,
    "switzerlandnorth": 0.25,
    "japaneast": 0.23,
    "japanwest": 0.23,
    "southeastasia": 0.23,
    "eastasia": 0.24,
    "australiaeast": 0.25,
    "australiasoutheast": 0.25,
    "brazilsouth": 0.28,
}
        ...
        for item in data.get("Items") or []:
            ...
            if (
                ...
                if price > 0:
                    ...
                    log.info(
                        "Azure Files Premium LRS price %s: $%.4f/GiB/mo (live API)",
                        region,
                        price,
                    )
                    return price
...
_SPOT_PRICES: dict[str, dict[str, float]] = {
    "Standard_D2s_v3": {
        ...
        "northeurope": 0.0420,
        "westeurope": 0.0410,
        "swedencentral": 0.0400,
        "japaneast": 0.0390,
        "southeastasia": 0.0380,
        "australiaeast": 0.0420,
        "uksouth": 0.0410,
        "eastasia": 0.0390,
    },
    "Standard_D4s_v3": {
        ...
        "northeurope": 0.0840,
        "westeurope": 0.0820,
        "swedencentral": 0.0800,
        "japaneast": 0.0780,
        "southeastasia": 0.0760,
        "australiaeast": 0.0840,
        "uksouth": 0.0820,
        "eastasia": 0.0780,
    },
    "Standard_D8s_v3": {
        ...
        "northeurope": 0.1680,
        "westeurope": 0.1640,
        "swedencentral": 0.1600,
        "japaneast": 0.1560,
        "southeastasia": 0.1520,
    },
    "Standard_E4s_v3": {
        "eastus": 0.0852,
        "westus": 0.0852,
    ...
    "Standard_F1als_v7": {
        ...
        "northeurope": 0.0036,
        "westeurope": 0.0035,
        "swedencentral": 0.0034,
        "japaneast": 0.0034,
        "southeastasia": 0.0033,
        "australiaeast": 0.0036,
        "uksouth": 0.0035,
        "eastasia": 0.0034,
    },
}
...
_ONDEMAND_PRICES: dict[str, dict[str, float]] = {
    "Standard_D2s_v3": {
        ...
        "northeurope": 0.1056,
        "westeurope": 0.1056,
        "swedencentral": 0.1056,
        "japaneast": 0.1008,
        "southeastasia": 0.1008,
        "australiaeast": 0.1072,
        "uksouth": 0.1056,
        "eastasia": 0.1008,
    },
    "Standard_D4s_v3": {
        ...
        "northeurope": 0.2112,
        "westeurope": 0.2112,
        "swedencentral": 0.2112,
        "japaneast": 0.2016,
        "southeastasia": 0.2016,
        "australiaeast": 0.2144,
        "uksouth": 0.2112,
        "eastasia": 0.2016,
    },
    "Standard_D8s_v3": {
        ...
        "northeurope": 0.4224,
        "westeurope": 0.4224,
        "swedencentral": 0.4224,
        "japaneast": 0.4032,
        "southeastasia": 0.4032,
    },
    "Standard_E4s_v3": {
        "eastus": 0.2133,
        "westus": 0.2133,
    ...
    "Standard_F1als_v7": {
        ...
        "northeurope": 0.0104,
        "westeurope": 0.0101,
        "swedencentral": 0.0099,
        "japaneast": 0.0098,
        "southeastasia": 0.0097,
        "australiaeast": 0.0104,
        "uksouth": 0.0101,
        "eastasia": 0.0098,
    },
}
...
def _fetch_retail_price(
    vm_size: str, region: str, price_type: str = "Consumption"
) -> float | None:
    ...
    try:
        ...
        if want_spot:
            ...
            if items:
                return (
                    float(items[0].get("retailPrice") or items[0].get("unitPrice") or 0)
                    or None
                )
    except Exception as exc:
...
def _fetch_spot_price(vm_size: str, region: str) -> float | None:
    ...
    if vm_size in _ONDEMAND_PRICES and region_norm in _ONDEMAND_PRICES[vm_size]:
        ...
        _SPOT_PRICE_CACHE[cache_key] = price
        log.info(
            "✓ Spot price %s (%s): $%.4f/hr (40%% on-demand table)",
            vm_size,
            region_norm,
            price,
        )
...
def _fetch_ondemand_price(vm_size: str, region: str) -> float | None:
    ...
    if price is not None:
        _SPOT_PRICE_CACHE[cache_key] = price
        log.info(
            "✓ On-demand price %s (%s): $%.4f/hr (live API)", vm_size, region, price
        )
        return price
    ...
    if vm_size in _ONDEMAND_PRICES and region_norm in _ONDEMAND_PRICES[vm_size]:
        ...
        _SPOT_PRICE_CACHE[cache_key] = price
        log.info(
            "✓ On-demand price %s (%s): $%.4f/hr (table)", vm_size, region_norm, price
        )
        return price
...
def _fetch_resource_inventory() -> list[dict]:
    ...
    vmss_resources = [
        ...
        for r in all_res
        if r.id
        and (r.type or "").lower() == "microsoft.compute/virtualmachinescalesets"
    ]
    ...
    if vm_resources:
        try:
            ...
            for vm_res in vm_resources:
                ...
                try:
                    inst = cc.virtual_machines.get(
                        rg, vm_res.name, expand="instanceView"
                    )
                    ...
                    vm_size = (
                        (inst.hardware_profile.vm_size or "")
                        if inst.hardware_profile
                        else ""
                    )
                    if not vm_size:
                        try:
                            arm_res = rc.resources.get_by_id(
                                vm_res.id, api_version="2024-03-01"
                            )
                            props = arm_res.properties or {}
                            vm_size = (props.get("hardwareProfile") or {}).get(
                                "vmSize", ""
                            )
                        except Exception:
                            pass
                    ...
                    try:
                        nics = (
                            (inst.network_profile.network_interfaces or [])
                            if inst.network_profile
                            else []
                        )
                        if nics:
                            nic_res = rc.resources.get_by_id(
                                nics[0].id, api_version="2024-05-01"
                            )
                            nic_props = nic_res.properties or {}
                            ip_cfgs = nic_props.get("ipConfigurations") or []
                            if ip_cfgs:
                                private_ip = (ip_cfgs[0].get("properties") or {}).get(
                                    "privateIPAddress", ""
                                )
                    except Exception:
                        pass
        ...
        try:
            ...
            for vmss_res in vmss_resources:
                ...
                try:
                    ...
                    is_spot = (
                        (getattr(vmp, "priority", None) or "").lower() == "spot"
                        if vmp
                        else False
                    )
                    vm_meta[vmss_res.id.lower()] = {
                        "vm_size": vm_size,
                        "is_spot": is_spot,
                        "private_ip": "",
                    ...
                    log.info(
                        "VMSS %s: size=%s spot=%s instances=%d",
                        vmss_res.name,
                        vm_size,
                        is_spot,
                        instance_count,
    ...
    storage_accounts = [
        r
        for r in all_res
        if (r.type or "").lower() == "microsoft.storage/storageaccounts"
    ]
    if storage_accounts and _HAS_STORAGE_MGMT:
        try:
            ...
            for sa in storage_accounts:
                ...
                try:
                    ...
                    for share in shares:
                        ...
                        price_per_gib = _fetch_files_premium_price(
                            sa.location or "eastus"
                        )
            ...
            log.warning(
                "StorageManagementClient error during file share enumeration: %s", e
            )
...
async def _get_live_data() -> list[dict]:
    ...
    async with _live_lock:
        now = time.monotonic()
        if (
            "inv" not in _live_cache
            or (now - _live_cache.get("ts", 0.0)) > LIVE_CACHE_TTL
        ):
    ...
    if not df.empty and "C_RESOURCE_ID" in df.columns and "C_COST" in df.columns:
        ...
        if "C_DATE" in mdf.columns:
            ...
            if last_date:
                ld_agg = (
                    mdf[mdf["C_DATE"] == last_date]
                    .groupby("C_RESOURCE_ID")["C_COST"]
                    .sum()
                )
                last_day_cost_by_id = {
                    str(k): round(float(v), 4) for k, v in ld_agg.items() if v > 0
                }
    ...
    for r in inv:
        ...
        export_days = (
            days_by_id.get(rid, window_days) if export_cost is not None else window_days
        )
        export_hours_val = hours_by_id.get(rid) if export_cost is not None else None
        is_storage_account = (
            r.get("type") or ""
        ).lower() == "microsoft.storage/storageaccounts"
        has_live_shares = (
            is_storage_account and (r.get("name") or "").lower() in sa_with_live_shares
        )
        ...
        if export_cost is not None and export_days < 28:
            if is_storage_account and not has_live_shares:
                ...
                last_day_projected = (
                    round(last_day_cost * 30, 2) if last_day_cost else 0
                )
                ...
                export_hours_val and export_hours_val > 0 and r.get("category") == "vm"
            ):
        ...
        vm_is_active = (r.get("status") or "").lower() not in (
            "deallocated",
            "stopped",
            "vm deallocated",
            "vm stopped",
