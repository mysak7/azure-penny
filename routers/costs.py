...
async def api_resource_groups_list(app_filter: str = "") -> JSONResponse:
    ...
    from live_resources import _get_live_data, _live_cache, _live_lock, list_resource_groups  # noqa: PLC0415
    ...
    try:
        ...
        if not df.empty and "C_NAME" in df.columns and "C_COST" in df.columns:
            ...
            cost_rgs  = {str(k): round(float(v), 2) for k, v in by_rg.items() if str(k).strip()}
...
async def api_anomalies(rg: str = "", app: str = "") -> JSONResponse:
    ...
    try:
        ...
        spikes    = sorted([s for s in svc_rows if s["delta_pct"] is not None and s["delta_pct"] > 50],     key=lambda x: x["delta_pct"], reverse=True)
        new_svcs  = sorted([s for s in svc_rows if s["is_new"] and s["current_week_usd"] > 1],              key=lambda x: x["current_week_usd"], reverse=True)
        gone_svcs = sorted([s for s in svc_rows if s["vanished"] and s["prior_week_usd"] > 1],              key=lambda x: x["prior_week_usd"], reverse=True)
        rg_spikes = sorted([r for r in rg_rows  if r["delta_pct"] is not None and r["delta_pct"] > 50],     key=lambda x: x["delta_pct"], reverse=True)
